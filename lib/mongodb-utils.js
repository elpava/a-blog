 ...
 * @param {object} client The Mongodb client
 * @param {string} collection The collection name
 * @param {object} documents The required empty {} to return all documents on a collection or {key: value} to filter queried data
 * @param {object} document The required query object to return single document in collection
 * @param {object} projectFields tthe required fields to projection the specific data
...
export async function getPost(client, document, projectFields) {
  const db = client.db('blog');

  return db.collection('posts').findOne(document, projectFields);
}
// UPDATE
